...
# 工具路径
TOOL=$DacapoRoot'/ehcbuilder-jar-with-dependencies.jar'
...
function javaBaseline() {
  local projectDir='/home/cbc/jvms/baseline/hotspot_openjdk-11+28/'
...
#   4. 运行的参数，即 java -cp temp App ...（注意如果有多个单词，必须用双引号包含）
#   5. 是否调用 rethrow，是为 true，否为 false
...
function testThrow() {
  ...
  while [[ ! -e ./error.txt ||
    ...
    # 运行加入 throw 的 dacapo 得到新的 _DTJVM.log 和 error.txt
    timeout $TIMEOUT"s" java -cp temp:$DacapoLib $app $args 2>error.txt
    if [[ $? == 124 ]] {
      rm error.txt >/dev/null 2>&1
    }
